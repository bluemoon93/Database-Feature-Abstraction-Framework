...
public class I_Orders implements II_Orders{
...
public static final int COMMIT=-21, RB=-23, AUTOCOMMIT=-26,
	...
	public I_Orders(IBusinessManager man, ObjectOutputStream oos, ObjectInputStream ois, String crud, Integer sessionID, Integer querySRID, IBusinessSchemaController controller, Integer activeSequence) throws SQLException {
	if(!controller.validateExecution(activeSequence, "I_Orders.II_Orders")) {
		throw new SequenceException("II_Orders was used out of order!");
	}
	...
	@Override
	public void execute(IRCustomerID args0, int args1, Date args2, Date args3, Date args4, int args5, double args6, String args7, String args8, String args9, String args10, String args11, String args12) throws SQLException {
	if(!controller.validateExecution(activeSequence, "I_Orders.II_Orders")) {
		throw new SequenceException("II_Orders was used out of order!");
	}
	String execParams = this.params;
			...
			execParams = execParams.replaceFirst("\\?","\""+args4+"\"");
			execParams = execParams.replaceFirst("\\?",""+args5);
			execParams = execParams.replaceFirst("\\?",""+args6);
			execParams = execParams.replaceFirst("\\?","\""+args7+"\"");
			execParams = execParams.replaceFirst("\\?","\""+args8+"\"");
			execParams = execParams.replaceFirst("\\?","\""+args9+"\"");
			execParams = execParams.replaceFirst("\\?","\""+args10+"\"");
			execParams = execParams.replaceFirst("\\?","\""+args11+"\"");
			execParams = execParams.replaceFirst("\\?","\""+args12+"\"");
		try{
			oos.writeInt(3);
			oos.flush();
