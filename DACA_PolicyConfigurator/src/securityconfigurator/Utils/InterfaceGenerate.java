...
public class InterfaceGenerate {
    ...
    public static String getImports(ImportStatus sts) {
        String importStr = "";
    ...
    public static String CreateAdditionalInterface(Interfaces inter, Class itf) {
        String interstr = "";
        switch (inter) {
            case IRead:
                interstr += "package " + itf.getPackage().getName() + ";\n";
    ...
    public static String CreateInterface(Interfaces inter, BusinessEntity_Context BE, String Name) {
        String interstr = "";
    ...
    public static String getMethods(ImportStatus exsts, BusinessEntity_Context BE) {
        String med = "";
        if (exsts.equals(ImportStatus.iread)) {
            Method[] mtds = BE.getReadMethods();
            if (mtds.length > 0) {
                for (int i = 0; i < mtds.length; i++) {
                    med += "\n" + mtds[i].getReturnType().getSimpleName() + " " + mtds[i].getName() + "() " + "throws SQLException;\n";
                }
            }
        }
        if (exsts.equals(ImportStatus.iwrite)) {
            ...
            if (mtds.length > 0) {
                for (int i = 0; i < mtds.length; i++) {
                    med += "\n" + "void" + " " + mtds[i].getName() + "(" + mtds[i].getReturnType().getSimpleName() + " " + mtds[i].getName() + ")" + " throws SQLException;\n";
                }
            }
        }
        ...
        if (exsts.equals(ImportStatus.iupdate)) {
            ...
            if (mtds.length > 0) {
                for (int i = 0; i < mtds.length; i++) {
                    med += "\n" + "void" + " " + mtds[i].getName() + "(" + mtds[i].getParameterTypes()[0].getSimpleName() + " " + mtds[i].getName().substring(1, mtds[i].getName().length()) + ")" + " throws SQLException;\n";
                }
            }
        }
    ...
    public static String getAdditionalInterfaceMethods(ImportStatus exsts, Class itf) {
        String med = "";
        if (exsts.equals(ImportStatus.iread)) {
    ...
    public static String getInterfaceDeclaration(String interfacename, ArrayList extendList) {
        String IntDec = "";
